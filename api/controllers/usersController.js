...
async function entrar(req, res) {
    ...
    if (usuarioEncontrado) {
        ...
        if (usuarioEncontrado.senha === senhaCifrada) {
            res.json({token: jwt.sign({email: usuarioEncontrado.email}, process.env.SEGREDO, {expiresIn: '9m'}) }); 
        } else {
...
function renovar(req, res) {
    ...
    if (token) {
        try {
            ...
            res.json({ token: jwt.sign({ email: payload.email }, process.env.SEGREDO)}); // back2024_auth
        } catch (err) {
